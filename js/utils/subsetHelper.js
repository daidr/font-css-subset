...
const loadAndInitializeHarfbuzz = async () => {
    const {
        instance: { exports },
    } = await WebAssembly.instantiate(
        await fetch(location.origin + '/public/hb-subset.wasm').then(response => response.arrayBuffer())
    );
    // exports.memory.grow(1000);
    const heapu8 = new Uint8Array(exports.memory.buffer);
    _global.exports = exports
    _global.heapu8 = heapu8
};
...
async function subsetFont(
    ...
    if (!_global.exports) {
        throw new Error('Have to init first');
    }
    const { exports, heapu8 } = _global
    ...
    heapu8.set(originalFont, fontBuffer);
...
module.exports = {
    subsetFont,
    init: loadAndInitializeHarfbuzz
}